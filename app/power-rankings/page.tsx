...
function injuredCount(team: TeamRoster): number {
  return team.players.filter((p) => p.injuryStatus !== null).length
}
...
function buildMarkdown(teams: TeamRoster[]): string {
  ...
  return ['# Power Rankings — Dynastat', '*Dynasty · Half-PPR · 1QB · 12 Teams*', '', header, divider, ...rows].join('\n')
}
    ...
    <div className="px-3 py-3 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-3 gap-3">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">values refresh every 6h · tap a team to view roster</p>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-900 text-xs text-gray-400 border-b border-gray-800">
              ...
              <th className="text-right py-2 px-2 font-medium hidden sm:table-cell">Inj</th>
              <th className="text-right py-2 px-2 font-medium">Age</th>
            </tr>
          </thead>
          <tbody>
            {teams.map((team, i) => {
              const inj = injuredCount(team)
              return (
                <tr
                  key={team.rosterId}
                  className={`border-b border-gray-900 last:border-0 ${
                    team.isMe ? 'bg-blue-950/50' : i % 2 === 0 ? 'bg-gray-950' : 'bg-gray-950/50'
                  }`}
                >
                  ...
                  <td className="py-2.5 px-2">
                    {/* Full-cell tap target via block link */}
                    <Link href={`/team/${team.rosterId}`} className="block group">
                      <div
                        className={`font-medium text-sm leading-tight group-hover:underline ${
                          team.isMe ? 'text-blue-300' : ''
                        }`}
                      >
                        {team.teamName}
                  ...
                  <td className="py-2.5 px-2 text-right tabular-nums hidden sm:table-cell">
                    {inj > 0 ? (
                      <span className="text-red-400 text-xs font-medium">{inj}</span>
                    ) : (
      ...
      <p className="text-xs text-gray-600 mt-2 md:hidden text-center">
        QB · RB · WR · TE · injury counts on wider screens
      </p>
    </div>
  )
}
